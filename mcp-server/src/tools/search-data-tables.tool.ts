...
export const toolDescription = `
  Search for Census Bureau data tables by ID, label, or dataset. Use this tool when users reference a topic or variable category (e.g., "language spoken at home", "income by race") and need to identify the correct table ID before fetching data. Accepts a table ID prefix (e.g., "B16005"), a natural language label query, and an optional dataset scope. Returns a ranked list of matching tables with their canonical labels and the datasets in which they appear. The returned data_table_id maps directly to the get.group parameter in fetch-aggregate-data.
`
