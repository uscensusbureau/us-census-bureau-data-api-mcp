...
export class DescribeDatasetTool extends BaseTool<DescribeDatasetArgs> {
  ...
  private filterMetadataForDataset(
    ...
    if (!matchingDataset) {
      ...
      console.log(
        'Available datasets:',
        metadata.dataset.map((d) => d.c_dataset.join('/')),
      )
      return null
    }
   ...
   */
  private updateDatasetMetadataForTarget(
    dataset: DatasetType,
    targetDataset: string,
    ...
    if (targetPath.length > originalPath.length) {
      ...
      return {
        ...
        c_variablesLink: dataset.c_variablesLink.replace(
          originalPath.join('/'),
          targetPath.join('/'),
        ),
        c_geographyLink: dataset.c_geographyLink.replace(
          originalPath.join('/'),
          targetPath.join('/'),
        ),
        c_documentationLink: dataset.c_documentationLink.replace(
          originalPath.join('/'),
          targetPath.join('/'),
        ),
        ...(dataset.c_examplesLink && {
          c_examplesLink: dataset.c_examplesLink.replace(
            originalPath.join('/'),
            targetPath.join('/'),
          ),
        }),
        ...(dataset.c_groupsLink && {
          c_groupsLink: dataset.c_groupsLink.replace(
            originalPath.join('/'),
            targetPath.join('/'),
          ),
        }),
        ...(dataset.c_tagsLink && {
          c_tagsLink: dataset.c_tagsLink.replace(
            originalPath.join('/'),
            targetPath.join('/'),
          ),
        }),
        ...
        distribution: dataset.distribution.map((dist) => ({
          ...dist,
          accessURL: dist.accessURL.replace(
            originalPath.join('/'),
            targetPath.join('/'),
          ),
        })),
  ...
  private formatMetadataAsJson(
    ...
    const filteredMetadata = this.filterMetadataForDataset(
      metadata,
      targetDataset,
    )
    ...
    return {
      ...
      dataset: {
        ...
        availability: {
          ...
          ...(dataset.c_isTimeseries && {
            isTimeseries: dataset.c_isTimeseries,
          }),
        },
        coverage: {
  ...
  private async fetchMetadataFromEndpoints(
    args: DescribeDatasetArgs,
    apiKey: string,
  ) {
    ...
    console.log(
      `Attempting to fetch metadata from dataset endpoint: ${datasetUrl}`,
    )
    ...
    try {
      ...
      if (datasetResponse.ok) {
        ...
        console.log(
          'Raw response sample:',
          JSON.stringify(datasetData).substring(0, 200),
        )
        ...
        if (this.isValidMetadataResponse(datasetData)) {
          ...
          if (Array.isArray(datasetData) && datasetData.length > 0) {
            throw new Error(
              `Endpoint ${datasetUrl} returned data instead of metadata. This might be a data endpoint rather than a metadata endpoint.`,
            )
          }
        }
      } else {
        console.log(
          `Dataset endpoint returned: ${datasetResponse.status} ${datasetResponse.statusText}`,
        )
      }
    } catch (error) {
      console.log(
        `Dataset endpoint failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      )
      throw error
    }

    throw new Error(
      `Unable to retrieve metadata from any available endpoint for ${args.dataset} (${args.year}). This dataset may not have metadata available, or may be a data-only endpoint.`,
    )
  }
  ...
  async handler(
    ...
    try {
      ...
      if (this.isValidMetadataResponse(result.data)) {
        const formattedData = this.formatMetadataAsJson(
          result.data,
          args.dataset,
        )
        ...
        const responseWithSource = {
          ...
          ...(result.data.dataset[0].note && {
            note: result.data.dataset[0].note,
          }),
        }
        ...
        try {
          ...
          const validatedResponse =
            DatasetMetadataJsonSchema.parse(responseWithSource)
        ...
        return this.createErrorResponse(`Metadata validation failed.`)
      }
    } catch (error) {
      const errorMessage =
