...
} from '../types/geography-level.types.js'
...
export class FetchDatasetGeographyTool extends BaseTool<FetchDatasetGeographyArgs> {
  ...
  private async getGeographyLevels(): Promise<GeographyLevelRow[]> {
    const result = await this.dbService.query<GeographyLevelRow>(`
      SELECT 
        id,
        name,
        description,
  ...
  private buildGeographyMetadata(levels: GeographyLevelRow[]): GeographyMetadata {
    ...
    for (const level of levels) {
      ...
      if (level.parent_summary_level) {
        ...
        const parentLevel = levels.find(l => l.summary_level === level.parent_summary_level);
        if (parentLevel) {
  ...
  private parseGeographyJsonWithDb(rawGeography: unknown, geographyLevels: GeographyLevelRow[]): ParsedGeographyEntry[] {
    ...
    const codeToLevel = new Map<string, GeographyLevelRow>();
    ...
    const parsed = validatedRaw.fips.map((entry) => {
      ...
      const displayName = dbLevel?.name || this.getDisplayNameFromApiName(entry.name);
      const querySyntax = dbLevel?.query_name || entry.name.replace(/\s+/g, '+');
      const queryExample = metadata?.queryExample || this.generateFallbackQueryExample(entry.name, entry.requires);
  ...
  private generateFallbackQueryExample(name: string, requires?: string[]): string {
    ...
    const parentSyntax = requires.map(req => req.replace(/\s+/g, '+')).join(':*&in=');
  ...
  async handler(args: FetchDatasetGeographyArgs): Promise<{ content: ToolContent[] }> {
    try {
      ...
      if (!isDbHealthy) {
        return this.createErrorResponse("Database connection failed - cannot retrieve geography metadata.");
      }

      // Get geography levels from database
      const geographyLevels = await this.getGeographyLevels();
      ...
      if (geographyResponse.ok) {
        ...
        try {
          ...
          const parsedGeographyData = this.parseGeographyJsonWithDb(validatedData, geographyLevels);
