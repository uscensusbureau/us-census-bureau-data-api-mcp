...
describe('FetchDatasetGeographyTool - Integration Tests', () => {
  ...
  afterEach(async () => {
    ...
    await testClient.query('TRUNCATE geography_levels RESTART IDENTITY CASCADE');
  });
  ...
  describe('Real Census API Integration', () => {
    ...
    it('should work with timeseries datasets', async () => {
      ...
      expect(responseText).toContain('Available geographies for timeseries/healthins/sahie');
  ...
  describe('Database-Driven Metadata Enhancement', () => {
    ...
    it('should build correct hierarchical query examples', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          fips: [
            ...
            { name: 'state', geoLevelDisplay: '040', referenceDate: '2022-01-01', requires: ['us'] },
            { name: 'county', geoLevelDisplay: '050', referenceDate: '2022-01-01', requires: ['state'] },
            { name: 'congressional district', geoLevelDisplay: '500', referenceDate: '2022-01-01', requires: ['state'] }
          ]
        })
      });
      ...
      expect(congressionalRecord?.queryExample).toBe('for=congressional+district:*&in=state:*');
    });
    ...
    it('should handle mixed on_spine values from database', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          fips: [
            { name: 'us', geoLevelDisplay: '010', referenceDate: '2022-01-01' },
            { name: 'urban area', geoLevelDisplay: '400', referenceDate: '2022-01-01' }
          ]
        })
      });
