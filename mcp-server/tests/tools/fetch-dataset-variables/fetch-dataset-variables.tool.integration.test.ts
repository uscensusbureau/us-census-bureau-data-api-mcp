...
describe('FetchDatasetVariablesTool - Integration Tests', () => {
  ...
  it('should handle real API errors gracefully', async () => {
    ...
    expect(response.content[0].text).toContain(
      'Variables endpoint returned: 404',
    )
  }, 10000)
  ...
  describe('when a random dataset is fetched', () => {
    ...
    beforeAll(async () => {
      try {
        ...
        const catalog: CensusCatalogResponse =
          (await response.json()) as CensusCatalogResponse

        // Filter to only available datasets with variables links
        const availableDatasets = catalog.dataset.filter(
          (ds) =>
            ds.c_isAvailable &&
            ds.c_variablesLink &&
            ds.c_dataset &&
        ...
        randomDatasets.forEach((ds, i) => {
          console.log(
            `${i + 1}. ${ds.dataset}${ds.year ? ` (${ds.year})` : ''} - ${ds.title}`,
          )
        })
      } catch (error) {
      ...
      if (randomDatasets.length === 0) {
        throw new Error(
          'No datasets available for testing - API returned empty dataset list',
        )
      }
    }, 30000)
    ...
    it('should successfully fetch variables for all random datasets', async () => {
      ...
      for (const testDataset of randomDatasets) {
        try {
          ...
          const variableCountMatch = responseText.match(
            /Total Variables: (\d+)/,
          )
          const variableCount = variableCountMatch
            ? parseInt(variableCountMatch[1])
            : 0
      ...
      if (successful.length > 0) {
        ...
        successful.forEach((result) => {
          console.log(
            `  ✓ ${result.dataset}${result.year ? ` (${result.year})` : ''} - ${result.variableCount} variables`,
          )
        })
      }
      ...
      if (failed.length > 0) {
        ...
        failed.forEach((result) => {
          console.log(
            `  ✗ ${result.dataset}${result.year ? ` (${result.year})` : ''} - ${result.error}`,
          )
        })
      }
    ...
    it('should handle edge cases with random datasets', async () => {
      ...
      if (randomDatasets.length > 0) {
        ...
        console.log(
          `Future year test result: ${responseText.substring(0, 100)}...`,
        )
      }
    }, 30000)
  })
  ...
  it('should handle general edge cases and error conditions', async () => {
    ...
    expect(invalidResponse.content[0].text).toContain(
      'Variables endpoint returned:',
    )
  }, 30000)
})
