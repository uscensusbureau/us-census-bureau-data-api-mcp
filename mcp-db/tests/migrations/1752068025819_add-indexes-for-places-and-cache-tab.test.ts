...
import { up, down } from '../../migrations/1752068025819_add-indexes-for-places-and-cache-tables';
  ...
  describe('up', () => {
    it('should create indexes for places table', async () => {
      await up(mockPgm);
			...
			expect(createIndexSpy).toHaveBeenCalledWith('places', ['state_code', 'county_code']);
			...
			expect(createIndexSpy).toHaveBeenCalledWith('places', ['geography_code', 'place_type']);
			expect(createIndexSpy).toHaveBeenCalledWith('places', 'parent_place_id');
			expect(createIndexSpy).toHaveBeenCalledWith('places', ['latitude', 'longitude']);
			expect(sqlSpy).toHaveBeenCalledWith(
			...
			expect(createIndexSpy).toHaveBeenCalledWith('places', ['place_type', 'state_code', 'county_code']);
			expect(sqlSpy).toHaveBeenCalledWith(
    ...
    it('should create indexes for census_data_cache table', async () => {
      await up(mockPgm);
			...
			expect(createIndexSpy).toHaveBeenCalledWith('census_data_cache', 'request_hash');
		  expect(createIndexSpy).toHaveBeenCalledWith('census_data_cache', ['dataset_code', 'year']);
		  expect(createIndexSpy).toHaveBeenCalledWith('census_data_cache', 'expires_at');
		  expect(createIndexSpy).toHaveBeenCalledWith('census_data_cache', 'last_accessed');
		  expect(sqlSpy).toHaveBeenCalledWith(
  ...
  describe('down', () => {
  	it('should drop indexes', async () => {
  	  await down(mockPgm);
  		...
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_name');
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_full_name');
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_population');
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_active');
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_predecessor_geoid');
  		expect(sqlSpy).toHaveBeenCalledWith('DROP INDEX IF EXISTS idx_places_successor_geoid');
  	});
  });
});