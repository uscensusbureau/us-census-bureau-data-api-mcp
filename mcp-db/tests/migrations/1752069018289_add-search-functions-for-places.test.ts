...
describe('Migration 1752069018289 - Add Search Functions for Places', () => {
  ...
  describe('up', () => {
    ...
    it('should create search_places function', async () => {
      ...
      expect(normalizeSQL(sqlSpy.mock.calls[0][0])).toBe(normalizeSQL(searchPlacesSql));
    });

    it('should create fuzzy_search_places function', async () => {
      await up(mockPgm);
      
      expect(normalizeSQL(sqlSpy.mock.calls[1][0])).toBe(normalizeSQL(fuzzySearchPlacesSql));
    });

    it('should create resolve_geography_by_coordinates function', async () => {
      await up(mockPgm);
      
      expect(normalizeSQL(sqlSpy.mock.calls[2][0])).toBe(normalizeSQL(geoCoordinateSearchSql));
    });
  });
  ...
  describe('down', () => {
  	it('should drop triggers and functions', async () => {
  	  await down(mockPgm);
  		...
  		expect(sqlSpy).toHaveBeenCalledWith('DROP FUNCTION IF EXISTS resolve_geography_by_coordinates(DECIMAL, DECIMAL, DECIMAL)');
		  expect(sqlSpy).toHaveBeenCalledWith('DROP FUNCTION IF EXISTS fuzzy_search_places(TEXT, REAL, INTEGER)');
		  expect(sqlSpy).toHaveBeenCalledWith('DROP FUNCTION IF EXISTS search_places(TEXT, CHAR, TEXT[], INTEGER)');
  	});
  });
});