...
import { runSeedsWithRunner, runSeeds, seeds } from '../../../src/seeds/scripts/seed-database';
...
async function createSeedRunnerSpy(mockRunner: MockRunner) {
  return vi.spyOn(await import('../../../src/seeds/scripts/seed-runner'), 'SeedRunner')
...
describe('Seed Database', () => {
  ...
  describe('seeds', () => {
    describe('geography_levels seed configuration', () => {
      it('should have valid configuration structure', () => {
        const geographySeed = seeds.find(s => s.file === 'geography_levels.json');
        
        expect(geographySeed).toBeDefined();
        expect(geographySeed?.table).toBe('geography_levels');
        expect(geographySeed?.dataPath).toBe('geography_levels');
      ...
      describe('afterSeed logic', () => {
        ...
        it('should handle missing parent references gracefully', async () => {
          ...
          // Should not throw
          await expect(
            geographySeed!.afterSeed!(client)
          ).resolves.not.toThrow();
          
          // Verify orphan has null parent_geography_level_id
          const orphanResult = await client.query(`
            SELECT parent_geography_level_id 
            FROM geography_levels 
            WHERE summary_level = '999'
          `);
          
          expect(orphanResult.rows[0].parent_geography_level_id).toBeNull();
        });
        ...
        it('should log relationship statistics', async () => {
          ...
          try {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
              expect.stringContaining('Geography levels: 3 total, 2/2 with parents')
            );
          ...
          try {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(
              ...
              expect.arrayContaining([
                expect.objectContaining({ name: 'Orphan1', summary_level: '888' }),
                expect.objectContaining({ name: 'Orphan2', summary_level: '999' })
              ])
            );
          } finally {
            consoleSpy.mockRestore();
  ...
  describe('runSeeds', () => {
    ...
    it('should handle database connection errors', async () => {
      ...
      try {
        await expect(runSeeds('bad://url', customSeedConfigs)).rejects.toThrow('Connection failed');
    ...
    it('should pass through seed filtering to runSeedsWithRunner', async () => {
      ...
      try {
        ...
        await runSeeds(databaseUrl, customSeedConfigs, 'geography_levels.json');
    ...
    it('should handle error during seeding and still cleanup', async () => {
      ...
      try {
        await expect(runSeeds(databaseUrl, customSeedConfigs)).rejects.toThrow('Seeding failed');
  ...
  describe('runSeedsWithRunner', () => {
    it('should run the complete seeding process with provided runner', async () => {
      // Create the expected geography_levels.json file
      const testGeographyData = {
        geography_levels: [
          {
            name: "Nation",
    ...
    it('should handle specific seed file targeting', async () => {
      ...
      await fs.writeFile(
        path.join(__dirname, 'fixtures', 'geography_levels.json'),
      ...
      await runSeedsWithRunner(runner, testSeedConfigs, 'geography_levels.json');

      // Should only have the data from geography_levels.json
